...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!instructor) {
    return { title: 'Instructor Not Found — Ninja Cooks Dojo' };
  }

  const name = getMetafieldValue(instructor.metadata?.name) || instructor.title;
  return {
    title: `${name} — Ninja Cooks Dojo`,
    description: getMetafieldValue(instructor.metadata?.bio) || `Meet ${name} at Ninja Cooks Dojo.`,
  };
}
